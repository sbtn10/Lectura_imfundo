document.addEventListener('DOMContentLoaded', function () {
    ...
    const words = ["PUEBLO", "PEQUEÑO", "VILLAANGELA", "LASGRUTAS", "LASLEÑAS", "PROVINCIA", "ARGENTINA", "MONTAÑA", "NATURALEZA"];
    const gridSize = 13;
    ...
    words.forEach(word => {
        ...
        listItem.textContent = word.replace(/ /g, '');
    ...
    // Validar preguntas de verdadero y falso
    ...
    options.forEach(option => {
        option.addEventListener("click", function() {
            ...
            if (isCorrect) {
                ...
                this.innerText = "Correcto";
            } else {
                this.style.backgroundColor = "#dc3545";
                this.style.color = "#ffffff";
                this.innerText = "Incorrecto";
            }

            // Desactivar todos los botones después de hacer una selección
            this.closest(".options").querySelectorAll("button").forEach(button => {
                button.disabled = true;
            });

            // Comprobar si todas las preguntas han sido contestadas
            checkCompletion();
        });
    });
    ...
    function checkCompletion() {
        ...
        const allAnswered = Array.from(allOptions).every(button => button.disabled);

        if (allAnswered && sopaCompleta) {
            mostrarModalYSalida();
        }
    }
});